    ...
    if isinstance(metadata, diem_types.Metadata__RefundMetadata) and isinstance(
        ...
        if original_txn is None:
            logger.info(
                ...
                f"Sending refund back to sender {sender_address}"
            )
    ...
    if transaction.status == TransactionStatus.PENDING:
        try:
            ...
            if transaction.type == TransactionType.EXTERNAL:
                ...
                add_transaction_log(transaction_id, "On Chain Transfer of General Txn Complete")
                log_execution(
            ...
            if transaction.type == TransactionType.REFUND:
                ...
                add_transaction_log(transaction_id, "On Chain Transfer of Refund Txn Complete")
                log_execution(
                    "On chain transfer complete "
                    f"txid: {transaction_id} "
                    f"v: {jsonrpc_txn.version} "
